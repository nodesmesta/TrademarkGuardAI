...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (!selected) return;
    setLoading(true);
    fetch(`/api/products/${selected}/monitoring-results`)
      .then((r) => r.json())
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-24 h-1.5 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${p.scanned > 0 ? Math.min(100, (p.violations / p.scanned) * 100) : 0}%` }}
                            />
                          </div>
                          <span className="text-xs text-gray-500">
                            {p.scanned > 0 ? ((p.violations / p.scanned) * 100).toFixed(1) : 0}%
                          </span>
                        </div>
                      </td>
                    </tr>
                  ...
                  {rows.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="py-2 pr-4">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${PLATFORM_COLORS[r.platform] ?? 'bg-gray-100 text-gray-700'}`}>
                          {r.platform}
                        </span>
                      </td>
                      ...
                      <td className="py-2 pr-4">
                        <span className={r.violation_count > 0 ? 'text-red-600 font-semibold' : 'text-green-600'}>
                          {r.violation_count}
                        </span>
                      </td>
